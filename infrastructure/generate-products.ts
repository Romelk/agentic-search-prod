 ...
 * NO PYTHON - Pure TypeScript implementation
 */
...
function generateProduct(index: number): Product {
  ...
  const subcategory = random(categories[category as keyof typeof categories]);
  ...
  const tags = styleTags[category as keyof typeof styleTags];
